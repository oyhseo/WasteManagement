...
# Load Data
train_dir = r'D:\WasteClass\DATASET\TRAIN'
val_dir = r'D:\WasteClass\DATASET\TEST'
...
def plot_metrics(all_labels, all_preds, all_probs, classes):
    ...
    plt.scatter(fpr, tpr, c=thresholds, cmap='coolwarm', label='Thresholds', s=15)
    ...
    thresholds = np.append(thresholds, 1)  # Ensure the last threshold matches the maximum probability
    plt.plot(recall, precision, label=f'PR curve (AP = {average_precision_score(all_labels, all_probs):.3f})')
    plt.scatter(recall[:-1], precision[:-1], c=thresholds[:-1], cmap='coolwarm', label='Thresholds', s=15)
    plt.colorbar(label='Threshold')
    plt.xlabel('Recall')
    ...
    for i, class_name in enumerate(classes):
        ...
        plt.plot(fpr, tpr, label=f'Class {class_name} (AUC = {auc:.3f})')
    plt.plot([0, 1], [0, 1], 'k--')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
...
history_df = pd.DataFrame({
    ...
    'val_precision': [m['precision'] for m in training_history['val_metrics']],
    'val_recall': [m['recall'] for m in training_history['val_metrics']],
    'val_f1': [m['f1'] for m in training_history['val_metrics']],
    'val_auc_roc': [m['auc_roc'] for m in training_history['val_metrics']]
})
